...
`     = sigma((xi - xj) ^ 2) + sigma((yi - yj) ^ 2)`
因为x，y都是整数，所以(xi - xj) ^ 2，(yi - yj) ^ 2的取值是完全平方数。当xi - xj == k时，一共有多少个k ^ 2呢？答案是所有行数相差k的两行的点数相乘的和。设col[t]为第t列的点个数，row[t]为第t行的点个数，则上一句话的形式化表述如下：
...
这样的两个点阵是否存在？很不幸，这是存在的，还不难举出来。下面是我的反例：
`XXX XX <> X XXXX`
`X XXX  <> XXX X `
