...
【20190207】重写了这一题，显式定义了状态转移的函数，整个程序结构显得更漂亮了|
...
|计算几何，极坐标表示|
|:--|
|对于题干中的公式（其实就是光通量的计算公式，会的直接用就行），需要计算一下积分
`dI = k / r * cos(Alpha) * dl * h`
`=> I = k * h * Integral(dl * cos(Alpha) / r)`
此处`dl * cos(Alpha) / r`是比较难计算的一块，如果化为`dl * v / (l ^ 2 + v ^ 2)`的话（其中v为光源到边的距离），没有可用的积分公式，相当麻烦。但是有一个比较取巧的方法，利用近似，认为`dl * cos(Alpha) = r * dAlpha`（后半部分是弧长公式，利用数形结合理解此法更好），把它化为`dAlpha`即可轻松得出结果`I = k * h * (maxAlpha - minAlpha)`（此法的正确性可通过证明`dl * cos(Alpha) - r * dAlpha -> 0`来说明）
那么，只需要求出多边形覆盖的角度范围即可。把顶点转换为极坐标表示，求一下最值，考虑一下特殊情况如“蚊香形”，答案就算出来了|

- [X] 1032

|贪心|
|:--|
|一开始理解错了题意，以为“每组派人在议会轮值”这句话表示每次派的人是按顺序轮流来的，那么就要求出所有划分中最大的最小公倍数，递推式`f[n] = LCM(f[n - i], i)`，且在题目数据范围下f[n]得用高精度整数表示……我还硬是写出来了
但是我机智地下载了原题，看到了另一组测试数据，跑了一遍发现不对，仔细检查结果也没问题，最后重新读题才发现问题所在：那句话的意思是，每组可以随意派人。这样，求的就是使所有组的组员排列数（即组员数乘积）最多的划分方法。结果就很显然了：按照`2, 3, 4, ..., t`来划分，多余部分从后往前一个个分配到组|

- [X] 1033

|模拟|
|:--|
|需要模拟“磁盘整理”的过程。把一个磁盘块移动到目标位置时，若目标位置被占用，则需要挪开目标位置上的块；题目要求移动操作数最少，则目标位置上的块最好也被移动到它自己的目标位置上。这样，就是一个递归的过程了，注意处理一下成环的情况就行了|

- [X] 1034

|二分图匹配|
|:--|
|题目中，人在从一点走向另一点时，狗子的停留点可能有多个选项，但每个停留点只能用一次，这就构成了二分图匹配的问题：需要匹配人行路径上的边和狗子的停留点
在不记得模板的情况下写了个差不多的匈牙利，结果因为小细节WA了两次……总之还是加深了对匈牙利算法的记忆和理解|

- [X] 1035

|字符串|
|:--|
|暴力匹配字符串是可以过的……这题数据范围还是较小。更大范围的数据可能要用到后缀数组之类的？|
...
|:--|
|比较基础的DP题，递推式如下
`f[i] = max{f[j] + p[i] \| 0 <= j < i && valid(j) && valid(j->i)}`|

- [X] 1037

|动态规划，排列|
|:--|
|是比较麻烦的动态规划+排列题。首先，动规部分设计如下：
`f[i][j][UP] = f[i - 1][k][DOWN], j <= k < i`
`f[i][j][DOWN] = f[i - 1][k][UP], 0 <= k < j`
f[i][j][UP]表示共i个数字、开头为j且j的下一个数字大于j时，可行的情况数（上式中k == j也可以取到，因为此处认为f[i - 1]中[j, i - 1)的部分映射到f[i]中的[j + 1, i)部分）；f[i][j][DOWN]同理
然后就是根据动规结果决定序列。首先，对每个i，遍历j，一旦累积序号accum + f[i][j][DIR] >= 当前序号c则当前j是i位置的正确数字，记录之，处理c和DIR后前进到下一个i。这里面有几点需要注意：i == 0时DIR == UP和DIR == DOWN都需要考虑，但i > 0时得根据之前的DIR忽略当前不合法的DIR；若当前DIR == DOWN，遍历j从0开始，否则从上一个j开始；前进到下一个i时要切换DIR
最后根据上面的映射关系，从后往前重新映射答案序列，就得到了最后的结果|

- [X] 1038

|动态规划（状态压缩）|
|:--|
|注意到m的值不大，从左向右遍历时，如果压缩当前列之前的两列，则状态数最多有3 ^ 10 = 59049个（每行的状态012分别表示`OO`、`XO`、`XX`，因为是从左向右遍历`OX`也算在`XX`里），转移方程`f[i + 1][nextState] = max{f[i + 1][nextState], f[i][currentState] + chipsAdded}`，其中nextState和chipsAdded通过DFS枚举添加的小块来产生。枚举情况数实际上 < 2 ^ (m / 2 + 1) <= 2 ^ 6 = 64，上界时间复杂度O(n * 3 ^ m * 2 ^ (m / 2 + 1))满足要求
我也尝试了压缩三列（包括当前列）的做法，状态数4 ^ 10 = 1048576，可以省掉DFS，只需要从小到大枚举状态并向同层的f添加新的可行状态即可。实际表现不如压缩两列的方法，大概慢了一倍，分析是状态数有较多冗余（当前列的初始情况是固定的，比如没有障碍的一列，有障碍的状态只在添加小块后产生，那么这些状态（< 2 ^ 6）占所有有障碍状态（约2 ^ 10）的比例比较小），且存在重复更新同一状态的操作，浪费了一些时间。不过使用位运算等技巧还是能AC的|

- [X] 1039

|计算几何|
|:--|
|答案所在的光路一定通过管道的两个顶点，否则能转动它使结果更优。那么枚举光通过的两个顶点，判断是否是可行的光路，并求出与管道的交点即可
写起来要考虑精度问题，要写线段与直线求交，要考虑求交时的顶点问题，还是挺麻烦的
坑爹的一点是我以为DBL_MIN跟INT_MIN一个意思，结果它表示的其实是最接近0的双精度正浮点数……听取WA声一片。最后发现-DBL_MAX才是最小的双精度浮点数|

- [X] 1040

|搜索+剪枝|
|:--|
|粗一看题意类似于背包问题，但背包容量的上限不明，强行DP可能会跪……不过其他数据范围非常小，暴搜复杂度O(2 ^ m)在m <= 22的条件下完全撑得住，于是交了个暴力上去过了……
冷静分析了一下，既然不能DP，给暴搜剪枝一下也是可以的。说起来搜索的过程跟24点问题有点像，那么把它的常用剪枝技巧拿来试试吧：
1 按照收益从大到小排序订单
2 若后面的订单加起来都打不过当前答案，放弃当前搜索支
本来还可以再加一条跳过重复订单的，不过总数只有22，我就当出现几率不大吧……
剪枝后接近800ms的搜索瞬间变成16ms……然后我就没动力继续优化了，就这样吧|
